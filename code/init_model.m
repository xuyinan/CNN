function model = init_model(layers, input_size, output_size, disp)
...
% [output5_,act5_] = inference(model,input5);

% grad5 = calc_gradient(model, input5, act5, ones(size(output5)));
grad5_ = calc_gradient_(model, input5, act5, ones(size(output5)));

train_params = struct('learning_rate',0.01,'weight_decay',0);
updated_model = update_weights(model,grad5_,train_params);
updated_model_ = update_weights_(model,grad5_,train_params);
...
if disp
	...
	for i = 1:num_layers-1
		...
		if (length(act_size) == 2 && size(act5{i},1) == size(act1{i},1)) ...
			...
			display(act_size(1:end-1));
		else
			fprintf('Error in layer %d, size mismatch between different batch sizes\n',i);
			display('With batch size 5:');
			display(act_size(1:end-1));
			display('With batch size 1:');
			display(size(act1{i}));
		end
	end
	...
	display(output_size);
	display('(Batch dimension not included)');
end
